...
const HeroSection = () => {
  ...
  const handleAddCar = () => {
    if (auth.currentUser) {
      ...
      navigate("/login", { state: { from: "dashboard/add-car" } });
    }
  };
  ...
  return (
    <section className="relative bg-gray-900 text-white overflow-hidden mt-[-35px]">
        ...
        >
          <h1 className="text-3xl xs:text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-tight mb-6">
            ...
            <span className="bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 bg-clip-text text-transparent">
              <Typewriter
                words={["RentWheels", "Luxury Cars", "Premium "]}
                loop
                cursor
                cursorStyle="|"
                typeSpeed={80}
          ...
          <p className="text-gray-200 text-base sm:text-lg md:text-xl mb-8">
            {" "}
            Discover top-tier car rentals with trusted provider. Book easily,
            drive in style, and enjoy every journey with comfort and luxury.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center lg:justify-start">
            <motion.button
              whileHover={{ scale: 1.05 }}
              onClick={handleBrowse}
              className="bg-linear-to-r from-amber-500 to-yellow-500 text-gray-900 px-6 sm:px-8 py-3 sm:py-4 rounded-full font-bold shadow-2xl hover:shadow-amber-400 transition-all duration-300"
            >
              Browse Cars
            </motion.button>
            <motion.button
      ...
      <div className="absolute bottom-0 left-0 w-full h-32 bg-linear-to-t from-gray-900 to-transparent"></div>
    </section>
  );
};
