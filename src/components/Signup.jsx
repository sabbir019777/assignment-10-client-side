...
const Signup = ({ setUser }) => {
  ...
  const saveUserToDatabase = async (user, token, customName, customPhoto) => {
    const defaultPhoto = "https://i.ibb.co/v36X0vD/avatar.png";
    try {
      await axios.put(
        endpoint("/api/users"),
        {
          name: customName || user.displayName || "Anonymous User",
          email: user.email,
          photo: customPhoto || user.photoURL || defaultPhoto,
          role: "user",
        },
        {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      await saveUserToDatabase(res.user, token, form.name, form.photoURL);

      const signupUser = {
        name: form.name, 
        email: res.user.email,
        photo: form.photoURL || "https://i.ibb.co/v36X0vD/avatar.png",
        token,
      };
      ...
      <div className="w-full max-w-md bg-gray-800 rounded-3xl shadow-2xl p-8 md:p-10 border border-gray-700">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-amber-500 hover:bg-amber-600 text-gray-900 font-semibold py-3 rounded-xl transition duration-300 disabled:opacity-70 disabled:cursor-not-allowed shadow-lg"
          >
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-gray-700 hover:bg-gray-600 py-3 rounded-xl font-semibold flex items-center justify-center gap-3 transition duration-300 disabled:opacity-70 disabled:cursor-not-allowed text-white border border-gray-600"
        >
