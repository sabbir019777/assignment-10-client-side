...
const MyListings = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(endpoint(`/api/cars/${deleteCandidate._id}`), {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
  ...
  const handleUpdateSubmit = async (formData) => {
    if (!accessToken || !selectedCar) return;

    try {
      const response = await fetch(endpoint(`/api/cars/${selectedCar._id}`), {
        method: "PUT", 
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
      ...
      setCars((prev) =>
        prev.map((car) => (car._id === selectedCar._id ? { ...car, ...updatedCar } : car))
      );
    ...
    <section className="min-h-screen p-8 relative bg-gradient-to-br from-black via-gray-900 to-black overflow-hidden py-20">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5">
        ...
        <div className="text-center mb-16">
          <h1 className="text-5xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-pink-500 to-pink-700 uppercase tracking-wide mb-4">
        ...
        <div className="grid md:grid-cols-3 gap-10">
          {cars.length > 0 ? (
            cars.map((car) => (
              ...
              >
                <div
                  className={`absolute top-5 right-5 px-4 py-2 rounded-full text-sm font-semibold z-20 ${
                ...
                <div className="p-6 relative">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 px-3 py-2 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 transition"
                    >
                      Update
                    </button>
                    <button
                      onClick={() => handleDelete(car)}
                      className="flex-1 px-3 py-2 rounded bg-red-600 text-white font-semibold hover:bg-red-700 transition"
                    >
                      Delete
                    </button>
                  </div>
      ...
      {deleteCandidate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-gray-900 border-2 border-red-500/50 rounded-xl p-8 max-w-sm w-full relative shadow-2xl shadow-red-600/20 transition-all duration-300">
